...
- `AvailabilityApiClient`/the offer endpoint haven't been verified against a **live**
  `Lakbay.AvailabilityApi` (that needs MongoDB, which needs Docker, which was unreliable
  in this session's environment) — the code path is real and correctly wired to Redis
  caching, confirmed via the exact exception it throws when the upstream is unreachable.
